...
class BlammoBot(BaseBot):
    ...
    # timestamps_dict = {
    #     'trivia': datetime.datetime.min,
    #     'points': datetime.datetime.min,
    #     'roulette': datetime.datetime.min,
    #     'give': datetime.datetime.min,
    #     'randommessage': datetime.datetime.min,
    #     'scramble': datetime.datetime.min,
    # }
    ...
    def _check_trivia_guess(self, msg: Message, questions: list|tuple):
        ...
        if answer.lower() in msg.content.lower():
            ...
            similarity_ratio = SM(None, msg.content.lower(), questions[1].lower()).ratio()
    ...
    async def on_privmsg_received(self, msg: Message):
        ...
        if self.stream_online == True and self.stream_online_prev == False:
            ...
            logger.info(f'Stream went online at {datetime.datetime.now().isoformat()}. Bot sleeping.')
        if self.stream_online == False and self.stream_online_prev == True:
            await msg.reply(f'Wokege')
            logger.info(f'Stream went offline at {datetime.datetime.now().isoformat()}. Bot awake.')
        ...
        if 'lekw' in msg.content.lower():
            ...
            logger.info(f'{msg.author} triggered LEKW response with message: {msg.content}')

        if REPLY_NEXT != '':
            reply_next_copy = REPLY_NEXT
            REPLY_NEXT = ''
            logger.info(f'{msg.author} triggered REPLY_NEXT, delivered the message: {reply_next_copy}')
        ...
        if trivia_started:
            ...
            if similarity >= 0.92 and len(msg.content) < 250:     # allow 8% error for spelling, punctuation, etc. 
                trivia_started = False
                await msg.reply(f'[Trivia] @{msg.author} You answered the question correctly and got 10 points. FeelsGoodMan The answer was: \"{questions[1]}\"')
                ...
                trivia_delta = datetime.datetime.now() - timestamps.read('trivia_started')
                ...
                trivia_started = False
                await msg.reply(f'[Trivia] @{msg.author} You answered the question correctly and got 8 points. FeelsGoodMan The answer was: \"{questions[1]}\"')
                ...
                await msg.reply(f'[Trivia] @{msg.author} {msg.content} is close. [Similarity {100*similarity:.0f}%]')
                logger.debug(f'[Trivia] Similarity: {similarity}')
        ...
        if scramble_started:
            ...
            # if msg.content.lower() == scramble_word[1].lower():
                ...
                await msg.reply(f'[Scramble] @{msg.author} You answered the question correctly and got 10 points. FeelsGoodMan The word was \" {scramble_word[1]} \"')
                points.add_points(msg.author, 10)

                # >>> record section <<<
                scramble_delta = datetime.datetime.now() - timestamps.read('scramble_started')
                ...
                record.add_guess_string(SCRAMBLE_QID, msg.content)  # TODO: make sure msg.content is type string
                record.add_guess_similarity(SCRAMBLE_QID, 1)    # similarity always 1 for correct scramble
    ...
    @Command('trivia',
             help='Tricia came back FeelsStrongMan')
    async def cmd_trivia(msg: Message):
        ...
        while trivia_started is True:
            ...
            if t == TRIVIA_TIMEOUT and trivia_started is True:
                trivia_started = False
                await msg.reply(f'[Trivia] No one answered correctly. FeelsBadMan The answer was: {questions[1]}')
                # >>> record section <<<
    ...
    async def cmd_scramble(msg: Message):
        ...
        if scramble_started and delta > datetime.timedelta(seconds=QUIET_REMIND_TIME):
            ...
            logger.debug(f'Scramble command blocked -- scramble already running. Reminded {msg.author}.')
            return
        elif scramble_started and delta <= datetime.timedelta(seconds=QUIET_REMIND_TIME):
            logger.debug(f'Scramble command blocked -- scramble already running. \
...
since new scramble round started.')
        ...
        scramble_word = scramble.get_word()     # tuple of (scrambled word, UNscrambled word)
        ...
        while scramble_started is True:
            ...
            if t == SCRAMBLE_TIMEOUT and scramble_started is True:
                await msg.reply(f'[Scramble] No one answered correctly. FeelsBadMan The word was: \" {scramble_answer} \"')
                # >>> record section <<<
    ...
    async def cmd_stoptrivia(msg: Message):
        ...
        if trivia_started == True:
            ...
            logger.debug(f'Command stoptrivia run, but no trivia round is active (trivia_started is False)')
    ...
    async def cmd_top(msg: Message):
        ...
        await msg.reply(f'Corpa WineTime Top 5: {top_usernames[0]} ({top_points[0]}), {top_usernames[1]} ({top_points[1]}), {top_usernames[2]} ({top_points[2]}), {top_usernames[3]} ({top_points[3]}), {top_usernames[4]} ({top_points[4]})')
    ...
    async def cmd_losers(msg: Message):
        ...
        await msg.reply(f'GAMBA 📉: {top_usernames[0]} ({top_points[0]}), {top_usernames[1]} ({top_points[1]}), {top_usernames[2]} ({top_points[2]}), {top_usernames[3]} ({top_points[3]}), {top_usernames[4]} ({top_points[4]})')
    ...
    async def cmd_roulette(msg: Message):
        ...
        if fmt == 'percent' and wager > 100:
            await msg.reply(f'[Roulette] @{msg.author} Weirdge You cannot wager more than 100% of your points.')
            return
        
        if fmt in ('percent', 'points'):
            if wager < 0:
                await msg.reply(f'[Roulette] @{msg.author} Weirdge Wager must be positive.')
                return
            if wager == 0:
                await msg.reply(f'[Roulette] 🫵 ICANT @{msg.author} just tried to wager 0 points')
                return
        try:
            ...
            return
        if out == 'win':
            await msg.reply(f'[Roulette] @{msg.author} HYPERPOGGER You won {delta} points and now have {new_bal} points.')
            return
        elif out == 'lose':
            await msg.reply(f'[Roulette] @{msg.author} FeelsBadMan You lost {delta} points and now have {new_bal} points.')
            return
        elif out == 'not enough points':
            await msg.reply(f'[Roulette] @{msg.author} Weirdge You don\'t have enough points for that. Get your money up.')
            return 
        elif out == 'no points':
            await msg.reply(f'[Roulette] @{msg.author} Sadge You don\'t have any points.')
    ...
    async def cmd_give(msg: Message):
        ...
        if points.get_points(sender) < transfer_amount:
            await msg.reply(f'@{msg.author} Weirdge You can\'t give more points than you have.')
            return
        ...
        if outcome == 'success':
            ...
            logger.debug(f'final amount: {points.get_points(sender) + points.get_points(receiver)}')
            return
        else:
    ...
    @Command('submit', help='Submit a trivia question+answer or a scramble word. Do #submit help for more.')
    async def cmd_submit(msg: Message):
        ...
        if restart_scheduled or shutdown_scheduled:
            logger.warning(f'Blocked #submit. Cannot submit question while restart or shutdown is scheduled')
            return
    ...
    @Command('reload', help='Reload a database or functionality.',
            permission='admin', syntax='#reload <subcommand>')
    ...
    @Command(
        'logger', help='Interact with the logger system.',
        permission='admin', syntax='#logger <subcommand> [args]',
        )
    ...
    @Command('shutdown', permission='mod',
         aliases=['stop', 'exit', 'quit'], help='Shuts down the bot.')
    ...
    @Command('scheduleshutdown', permission='mod', help='Shuts down the bot whenever there are no games running.')
    async def cmd_scheduleshutdown(msg: Message):
        ...
        await msg.reply('MrDestructoid Shutdown scheduled. Will shutdown when no games are running.')
    ...
    #         aliases=['slur']
    # )
    # async def cmd_slurs(msg: Message):
    #     await msg.reply(f'@{msg.author} Concerned')
    ...
    async def cmd_hasan(msg: Message):
        responses = [
            ...
            'peepoHas', 'peepoHas', 'peepoHas', 'peepoHas', 'peepoHas', 
            'peepoHas', 'peepoHas', 'peepoHas', 'peepoHas', 'peepoHas', 
    ...
    @Command('silentecho', permission='admin', help='Echo but silent. Use from Diraction\'s chat.')
    async def cmd_silentecho(msg: Message):
        ...
        if REPLY_NEXT == '':
            ...
            logger.warning(f'Command #silentecho command not completed since REPLY_NEXT already full.')
            logger.warning(f'Attempted to write message: {content}')
            logger.warning(f'REPLY_NEXT: {REPLY_NEXT}')



    @Command('dumptologs', permission='admin', help='Dumps most variables to logs for debugging.')
    ...
    async def on_whisper_received(self, msg: Message):
        ...
        logger.info(f'WhisperReceived: {msg.author} sent whisper {msg.content} to channel {msg.channel_name}')
