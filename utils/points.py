...
class PointData():
    ...
    def _validate_user(self, username: str):
        ...
        if username not in self.df['username'].values:
            ...
            columns=['username', 'points', 'gamble_loss']
            )
    ...
    def add_gamble_loss(self, username: str, num: int):
        ...
        self._validate_user(username)
        if num < 0:     # cannot have negative num
            logging.warning(f'Prevented add negative gamble loss to {username}. (num={num})')
            return
    ...
    def sub_points(self, username: str, num: int):
        ...
        points = self._user_points(username)
        if num < 0:   # cannot have negative num
            logging.warning(f'Prevented subtract negative points from {username}. (num={num})')
            return

        points_new = points - num
        if points_new < 0:
            points_new = 0      # cannot have negative points, so hard floor at 0
            logging.warning(f'Prevented {username} from having negative points. \
Tried to subtract {num} points from balance of {points}. Balance set to 0.')
    ...
    def transfer(self, sender: str, receiver: str, amount: int):
        ...
        if amount <= 0:
            logger.warning(f'Prevented transfer of {amount} points from {sender} to {receiver}. Amount cannot be <= 0.')
            logger.debug(f'sender bal: {sender_bal}, receiver bal: {receiver_bal}')
            return 'invalid amount'
        
        initial_bal = sender_bal + receiver_bal
        if sender_bal < amount:
            logger.warning(f'Prevented transfer of {amount} points from {sender} to {receiver}. Sender has insufficient balance.')
            logger.debug(f'sender bal: {sender_bal}, receiver bal: {receiver_bal}')
            return 'not enough points'
        
        if sender == receiver:
            logger.warning(f'Prevented transfer of {amount} points from {sender} to {receiver}. Cannot transfer to self.')
            logger.debug(f'sender bal: {sender_bal}, receiver bal: {receiver_bal}')
            return 'cannot transfer to self'
        
        if sender_bal < 0 or receiver_bal < 0:
            logger.error(f'Prevented transfer of {amount} points from {sender} to {receiver}. Sender or receiver has \
negative balance. {sender} sender bal: {sender_bal}, {receiver} receiver bal: {receiver_bal}')
        ...
        if initial_bal != final_bal:
            logger.error(f'Problem transfering {amount} points from {sender} to {receiver}. Initial balance sum ({initial_bal}) \
does not match final balance sum ({final_bal}). Forbidden.')
            ...
            if self.get_points(sender) + self.get_points(receiver) == initial_bal:
                ...
                logger.error(f'Could not revert changes. Final balance sum ({self.get_points(sender) + self.get_points(receiver)})')
                return 'balance mismatch and could not revert'
        else:
            logger.info(f'Successfully transfered {amount} points from {sender} to {receiver}.')
            return 'success'
    ...
    def gamble(
        ...
        if fmt not in ['points', 'percent', 'all']:
            raise ValueError(f'fmt must be one of ["points", "percent", "all"], not {fmt}')
        ...
        if fmt == 'points':
            ...
            outcome = random.choices(['win', 'lose'], weights=[win_odds, 1 - win_odds])[0]
            if outcome == 'win':
