...
def fuzzy_check(row):
    ...
    # Examples:
    #   - Question or answer contains slur, 
...
class TriviaData():
    ...
    def reload(self):
        if self.allow_load:
            ...
            self.df = pd.read_csv(self.path, index_col=False, header=0, encoding='latin-1')
    ...
    def question(self):
        ...
        while not found_valid:
            ...
            if q['enabled'] == True or q['enabled'] == 'TRUE' and str(q['question']) != 'nan' and str(q['correct_answer']) != 'nan':
                if 'not' not in str(q['question']):      # TEMPORARY FIX!
                    found_valid = True
            else:
                q_df.to_csv('rejected_questions.csv', mode='a', header=False, index=False)
                hbar = '='*81
        ...
        if str(q['correct_answer'])[-1] == 'Ê':
            ...
            logger.warning(f'Removed invalid character from answer: {q["correct_answer"]}')
            removed_char = True
        if removed_char:
            logger.info(f'Question: {q["question"]}')
            logger.info(f'Answer: {q["correct_answer"]}')
