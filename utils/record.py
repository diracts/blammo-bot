...
class Record:
    def __init__(
        ...
        self.cols=[
                'timestamp', 'qid', 'game_type', 'outcome', 'time_elapsed', 
                'username', 'points_awarded', 'guess_string', 
                'guess_similarity', 'question_string', 'answer_string'
            ]
    ...
    def new(self, qid: str) -> None:
        new = pd.DataFrame([[
            self.init_timestamp, self.init_qid, self.init_game_type,
            self.init_outcome, self.init_time_elapsed, self.init_username,
            self.init_points_awarded, self.init_guess_string,
            self.init_guess_similarity, self.init_question_string,
    ...
    def add_outcome(self, qid: str, outcome: str) -> None:
        if outcome not in ('solved', 'timeout'):
            logger.error(f'Invalid outcome passed to add_outcome for qid {qid}: {outcome}')
